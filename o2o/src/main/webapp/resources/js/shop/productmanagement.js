$(function(){
		...
		$.getJSON(listUrl, function(data){
			if(data.success){
				...
				productList.map(function(item,index){
					var textOp = "下架";
					var constraryStatus = 0;
					if(item.enableStatus == 0){
						//若状态值为0，表明商品已下架，操作变为上架(即点击上架按钮上架相关信息)
						textOp = "上架";
						constraryStatus = 1;
					}else {
						constraryStatus = 0;
					...
					+ item.productName + '</div><div class="col-20">' + item.priority +
					...
					+ constraryStatus + '">' + textOp + '</a>'
					+'<a href="#" class="preview" data-id="'
					+ item.productId + '" data-status="' + item.enableStatus + '">预览</a>'
					+'</div></div>';
				});
				//将拼接好的信息赋值进html控件中
	...
	$('.product-wrap').on('click','a',function(e){
		...
		if(target.hasClass('edit')){
			//如果有classedit，则点击进入店铺信息编辑页面，并带有productId参数
			...
			//如果有class priview，则调去前台展示系统给商品详情页，预览商品情况
			window.location.href='/o2o/frontend/productdetail?productId='
				+ e.currentTarget.dataset.id;
		};
	});
	function changeItemStatus(id, enableStatus){
		//定义product json对象，并添加productId以及状态(上架/下架)
		var product = {};
